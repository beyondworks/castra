...
## Castra
SessionStart injects the execution contract and this session's runtime pointers. `/castra` (standalone) or `/castra:castra` (plugin) supports run, review, verify, resume and status. A literal `castra: <mode> <request>` also routes through UserPromptSubmit.
